...
def maybe_start_remote_g2_target_client(
    ...
    *,
    ...
    # wrapper may not expose get_slot_idx_by_block_id.
    _kv_impl = getattr(kv, "impl", kv)
    window_size = _derive_window_size(_kv_impl)

    # Require the non-pinning accessor. pin_blocks_by_id is intentionally not a
    # fallback for remote-G2 because it can bypass radix-tree visibility.
    _has_get_slot = hasattr(_kv_impl, "get_slot_idx_by_block_id")
    logging.warning(
        "remote_g2: block_id_to_slot_idx setup: "
        "has_get_slot=%s window_size=%s kv_type=%s",
        _has_get_slot,
        window_size,
    ...
    def _block_id_to_slot_idx(block_ids: list[int]) -> list[int]:
        ...
        if not _has_get_slot:
            logging.warning("remote_g2: no non-pinning block_id→slot_idx method available")
            return []
        try:
            return [
    ...
    if adapter is None:
        logging.warning(
            ...
            "configured remote-G2 target client cannot start"
        )
        return False
